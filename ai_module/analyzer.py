...
def analyze_conversation(conv: Conversation) -> Dict[str, Any]:
    ...
    user_prompt = (
        ...
        "3. INSTRUCTION_FOR_WRITER: Give a specific, actionable command for the copywriter. FIRST: Check for Silent Objections (Busy, Cost, Imposter). If detected, use the corresponding TACTIC immediately - this overrides standard rapport questions. \n"
        "   IF IN 'doing_the_ask' PHASE: Check if the prospect is asking questions (info requests, price questions, objections). If so, use the SALES CONVERSATION MANAGEMENT tactics - DO NOT repeat the pitch. Answer their question directly, then ask a follow-up.\n"
        "   Otherwise, provide general guidance like:\n"
        "   - 'Acknowledge the price concern, but deflect by asking about their specific project interest first.'\n"
        "   - 'They're showing interest - introduce Prodicity naturally by connecting it to their mentioned project.'\n"
        "   - 'Continue building rapport - ask about their school or current projects to deepen the relationship.'\n"
